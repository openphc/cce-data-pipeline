...
| `schema/01`–`schema/08` `*.sql` | ClickHouse | applied once (in order) | Base tables (15, incl. `facility`, `step_sla_state_transitions` + `*_history`) → Kafka-engine queues + consumer MVs → aggregation MVs → indexes → dictionaries → current-state rollups → daily-summary refreshable MVs (08 is documentation-only) |
